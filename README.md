...
To demonstrate the robustness of the out-of-the-box scripts without the private dataset, we provide a structured set of AI-generated sample images. Our Python script replicates these samples to provide enough volume to satisfy the neural network validation splits, letting you evaluate our OpenCV sorters and YOLO architectures instantly.

1. Install dependencies: `pip install -r requirements-yolo.txt`
2. Run the dataset populator:
   ```bash
   python scripts/generate_dummy_data.py
   ```
   *(This populates `data/raw/` with 60 images by multiplying the provided reference samples — 30 conforming and 30 containing artificial 'defect' hotspots).*
3. Test the interactive sorter on the sample data:
   ```bash
   python scripts/classify_images.py
   ```
   *(Allows you to functionally sort the 60 sample images into `good` or `bad` folders via keyboard shortcuts).*
...
| Nominal Part (No Defects) | Defective Part (YOLO Detection) |
| :---: | :---: |
| <img src="docs/yolo_nodefect_example.png" height="350" alt="Nominal Part (No Defects)" /> | <img src="docs/yolo_defect_example.png" height="350" alt="Defective Part (YOLO Detection)" /> |
